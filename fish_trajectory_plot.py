...
for i in range(df1.size):
    if(i % random.randint(1,3) == 0):
        df1['X1'] = (df1['X1'] - df1['X1'].min()) / (df1['X1'].max() - df1['X1'].min()) * (xmax - xmin) + xmin + random.random()*0.1*random.randint(1,5)
        df1['X2'] = (df1['X2'] - df1['X2'].min()) / (df1['X2'].max() - df1['X2'].min()) * (xmax - xmin) + xmin + random.random()*0.1*random.randint(1,4)
        df1['Y1'] = (df1['Y1'] - df1['Y1'].min()) / (df1['Y1'].max() - df1['Y1'].min()) * (ymax - ymin) + ymin - random.random()*0.1*random.randint(1,6)
        df1['Y2'] = (df1['Y2'] - df1['Y2'].min()) / (df1['Y2'].max() - df1['Y2'].min()) * (ymax - ymin) + ymin + random.random()*0.01*random.randint(1, 5)
